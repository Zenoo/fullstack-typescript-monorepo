  ...
  const onSubmit = async (data: FormData) => {
    ...
    if (data.password) {
      if (data.passwordConfirm) {
        setLoading(true);
        await UserRoutes.changePassword(auth.user.id, data.password).catch(
          catchError(Alert)
        );
      }
    }
            ...
            <Grid item md={6} xs={12}>
              <TextField
                {...register('passwordConfirm', 'password', {
                  validate: {mustMatch: value => value === password},
                })}
                fullWidth
              />
            </Grid>
            <Grid item xs={12} sm={6}>
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  defaultValue={auth.user.lang || DEFAULT_LANGUAGE}
                >
