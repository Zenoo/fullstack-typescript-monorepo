...
import { GridToolbarColumnsButton, GridToolbarContainer, GridToolbarContainerProps, GridToolbarDensitySelector, GridToolbarFilterButton } from '@mui/x-data-grid';
    ...
    <GridToolbarContainer {...rest}>
      ...
      {importMethod && (
        <DropzoneDialog
          ...
          options={{ maxFiles: 1 }}
        >
          <Button size="small" startIcon={<IosShare />}>{t('import')}</Button>
        </DropzoneDialog>
      )}
      {empty && (
        <Button size="small" onClick={confirmEmpty} startIcon={<Delete />}>{t('empty')}</Button>
      )}
