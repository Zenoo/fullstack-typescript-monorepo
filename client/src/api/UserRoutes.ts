...
const UserRoutes = {
  ...
  >('user'),
  table: (state: TableState, include?: Prisma.UserInclude) =>
    Fetch<{data: UserWithPerson[]; count: number}>(
      '/api/user/table',
      {
        state,
        include,
  ...
  changePassword: (id: number, password: string) =>
    Fetch(`/api/user/${id}/change-password`, {password}, 'POST'),
  sendPasswordResetMail: (login: string) =>
    Fetch(`/api/user/${login}/send-password-reset-mail`),
  checkResetCodeValidity: (login: string, code: string) =>
