import {
  ExpectedError,
  PrismaInclude,
} from '@fullstack-typescript-monorepo/core';
...
const insert =
  (model: string) =>
  (prisma: PrismaClient) =>
  async (req: Request<never, unknown, unknown>, res: Response) => {
    try {
...
const getAllAsCsv =
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const objects = (await prismaModel.findMany()) as Record<
        string,
        unknown
      >[];
...
const table =
  (model: string) =>
  (prisma: PrismaClient) =>
  async (req: Request<never, unknown, TableRequestBody>, res: Response) => {
    try {
      await auth(prisma, req);

      const prismaModel = (prisma as GenericPrisma)[model] as MOCK_PrismaModel;

      const data = await TableUtils.getData(
        req,
        prismaModel,
        undefined,
        req.body.include
