...
import MailUtils, { MAIL_SENDER } from '../utils/MailUtils.js';
...
const authenticate = (prisma: PrismaClient) => async (
  ...
  try {
    ...
    if (passwordIsToken) {
      ...
      if (dayjs().isBefore(UserUtils.getTokenExpiration(user))) {
    ...
    if (passwordIsCorrect) {
      ...
      user.connexionToken = Buffer.from(`${Math.random().toString(36).substring(2, 15)}|${dayjs().add(7, 'days').toISOString()}`).toString('base64');
